...
from common import print_sleep, valueForKey, random_all_string, xor, print_error, print_info, print_success, print_warn, print_normal, print_all, PrintColor
...
class HKPIC(Network):
    ...
    def getHost(self):
        ...
        print_info('获取比思域名', self.nickname)
        ...
        if type(req) is dict and 'content' in req.keys():
            ...
            if not content:
                print_error(f'{self.nickname}:获取域名失败', self.nickname)
                return
            ...
            print_info(f'域名：{len(self.all_host)} 个', self.nickname)
        else:
            print_error(f'{self.nickname}:获取域名失败', self.nickname)

    # 测试域名
    def checkHost(self):
        print_info('测试域名', self.nickname)
        for host in self.all_host:
            html = self.forum(host=host, check_host=True)

            if html == '域名不通':
                print_error(f'{self.nickname}:{host} 请求失败，切换下一个域名', self.nickname)
                print_sleep(1)
                continue
    ...
    def login(self, show=False):
        ...
        print_success(('登录成功' if self.formhash else f'登录失败'), self.nickname)
    ...
    def signIn(self):
        ...
        items = re.findall(pattern, html)
        if items:
            print_success(items[0], self.nickname)
        else:
    ...
    def forum_list(self, first_time=False):
        ...
        if first_time:
            print_info(f'开始评论。\n每次评论需要间隔{self.config.reply_sleep_time}秒。', self.nickname)
            # 第一次时，先获取一下现有金币数
            self.myMoney(False)
        ...
        if span and span.text:
            print_info(f'进入版块：{span.text}「{fid}」', self.nickname)
    ...
    def reply(self, comment, fid, href):
        ...
        if not tid:
            print_error(f'{self.nickname}:帖子id不存在', self.nickname)
            return False
        ...
        print_info(f'进入帖子：{url}', self.nickname)
        # 发表评论前的金币数
        ...
        if html.find('非常感謝，回復發佈成功') > -1:
            ...
            self.config.save()
            print_success(f'第{self.config.reply_times}条：「{comment}」-> 發佈成功', self.nickname)
            self.myMoney(False)
            if money_history == self.my_money:
                # 如果发表评论后，金币数不增加，就不再发表评论
                print_warn('评论达到每日上限。不再发表评论。', self.nickname)
                self.config.reply_times = 9999
                self.config.save()
            return True
        elif html.find('抱歉，您所在的用戶組每小時限制發回帖') > -1:
            print_warn('评论数超过限制', self.nickname)
            self.config.last_reply_time = time.time()
            ...
            items = re.findall(pattern, html)
            print_error('\n'.join([comment] + items) if items else html, self.nickname)
            print_error(f'{self.nickname}:发表评论失败', self.nickname)
    ...
    def visitUserZone(self):
        ...
        if self.user_href:
            url = self.user_href
            print_info(f'访问别人空间：{url}', self.nickname)
            ...
            print_error(f'{self.nickname}:别人空间地址为空', self.nickname)
    ...
    def leavMessage(self, uid, fail_time: int = 0, is_faild: bool = False):
        ...
        if not uid:
            print_error(f'{self.nickname}:他人id为空', self.nickname)
            return
        ...
        if not is_faild:
            ...
            print_error(f'{self.nickname}:发表留言失败', self.nickname)
            return
        ...
        if html.find('操作成功') > -1:
            is_faild = False
            print_success('留言成功', self.nickname)
            self.config.is_leave_message = False
            self.config.save()
            ...
            pattern = re.compile(r'\[CDATA\[(.*?)<', re.I)
            items = re.findall(pattern, html)
            print_error('\n'.join(items) if items else html, self.nickname)
            print_error(f'{self.nickname}:留言失败', self.nickname)
    ...
    def deleteMessage(self, cid):
        ...
        html = self.request(url, params)
        if html.find('操作成功') > -1:
            print_success('删除留言成功', self.nickname)
            print_sleep(2)
        else:
            pattern = re.compile(r'\[CDATA\[(.*?)<', re.I)
            items = re.findall(pattern, html)
            print_error('\n'.join(items) if items else html, self.nickname)
            print_error(f'{self.nickname}:删除留言失败', self.nickname)
    ...
    def myMoney(self, is_print=True):
        ...
        if items:
            ...
            self.my_money = money
            if is_print:
                print_info(f'金钱：{money}', self.nickname)
        else:
            print_error(f'{self.nickname}:获取金币失败', self.nickname)
    ...
    def delAllLeavMessageDynamic(self):
        ...
        print_info('获取留言动态', self.nickname)
        ...
        if feedids:
            print_success(f'{len(feedids)}条动态', self.nickname)
        else:
            return
    ...
    def delLeavMessageDynamic(self, feedid, referer):
        ...
        html = self.request(url, params)
        if html.find('操作成功') > -1:
            print_success('一条删除动态成功', self.nickname)
        else:
            pattern = re.compile(r'\[CDATA\[(.*?)<', re.I)
            items = re.findall(pattern, html)
            print_error('\n'.join(items) if items else html, self.nickname)
            print_error(f'{self.nickname}:删除动态失败', self.nickname)
    ...
    def journal(self, money_history=None, fail_time: int = 0, is_fail: bool = False):
        ...
        if not is_fail:
            ...
            print_error(f'{self.nickname}:发表日志失败', self.nickname)
            return
        ...
        if html.find(title) > -1:
            ...
            self.config.save()
            print_success(f'第{self.config.journal_times}篇日志：「{title}」-> 發佈成功', self.nickname)
            self.myMoney(False)
            if money_history == self.my_money:
                # 如果发表后，金币数不增加，就不再发表
                print_warn('发表日志达到每日上限。', self.nickname)
    ...
    def allJournals(self, is_show):
        ...
        for id in ids:
            if id and id[2].startswith('我的日志'):
                if is_show:
                    print_info(f'日志：{id[1]}->「{id[2]}」', self.nickname, PrintColor.White)
    ...
    def delJournal(self, all_blogids=None, del_time=0):
        ...
        if all_blogids:
            ...
            print_error(f'{self.nickname}:没有需要删除的日志', self.nickname)
            return
        ...
        if blogid not in all_blogids:
            print_success(f'日志删除成功:「{blogid}」', self.nickname)
        else:
            del_time += 1
            print_error(f'{self.nickname}:日志删除失败:「{blogid}」', self.nickname)
        if not all_blogids:
            return
    ...
    def share(self, fail_time: int = 0, is_fail: bool = False):
        ...
        if html.find('操作成功') > -1:
            ...
            if money_history == self.my_money:
                # 如果发表后，金币数不增加，就不再发表
                print_warn('发表分享达到每日上限。', self.nickname)
                self.config.share_times = 9999
                self.config.save()
            else:
                print_success('发布分享成功。', self.nickname)
            ...
            items = re.findall(pattern, html)
            print_error('\n'.join(items) if items else html, self.nickname)
            print_error(f'{self.nickname}:发布分享失败', self.nickname)
            for item in items:
    ...
    def delShare(self, sid):
        ...
        if html.find('操作成功') > -1:
            print_success('删除分享成功', self.nickname)
        else:
            pattern = re.compile(r'\[CDATA\[(.*?)<', re.I)
            items = re.findall(pattern, html)
            print_error('\n'.join(items) if items else html, self.nickname)
            print_error(f'{self.nickname}:删除分享失败', self.nickname)
